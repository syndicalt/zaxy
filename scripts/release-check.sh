...
usage() {
    cat <<USAGE
Usage: scripts/release-check.sh [--root PATH] [--ruff-cmd CMD] [--mypy-cmd CMD] [--pytest-cmd CMD] [--coverage-cmd CMD] [--packet-smoke-cmd CMD] [--examples-smoke-cmd CMD] [--mcp-smoke-cmd CMD] [--langgraph-smoke-cmd CMD] [--coordinate-smoke-cmd CMD] [--package-cmd CMD] [--docs-cmd CMD] [--validate-cmd CMD] [--hook-status-cmd CMD] [--backend-shootout-cmd CMD] [--backend-performance-cmd CMD] [--backend-scale-cmd CMD] [--beta-uat-cmd CMD] [--external-validation-report PATH] [--external-validation-cmd CMD] [--require-external-validation]

Runs ruff, mypy, the full pytest suite, coverage ratchet, packet-memory smoke coverage, public examples, MCP smoke, LangGraph smoke, Coordinate mission smoke, packaging validation, docs validation, deployment validation, activation guardrails, backend shootout validation, medium-scale backend performance validation, 100-query backend scale validation, beta UAT, and optional external validation evidence.
