...
| `zaxy memory append`, `zaxy memory bootstrap`, `zaxy memory checkout`, `zaxy memory status`, `zaxy memory log`, `zaxy memory diff`, `zaxy memory refs` | `Beta` | Model-facing memory writes plus Eventloom inspection. The CLI `memory append` is the daemon/agent twin of the MCP `memory_append` tool and writes byte-identical events through the same `MemoryFabric.append` pipeline. |
